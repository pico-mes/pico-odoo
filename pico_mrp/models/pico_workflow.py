...
def pico_api(env):
    ...
    if not pico_url:
        raise ValidationError(
            'Creating Pico API requires a config parameter "pico.url"'
        )
...
class PicoMESWorkflow(models.Model):
    ...
    process_ids = fields.One2many(
        "pico.workflow.process", "workflow_id", string="Child Processes"
    )
    version_ids = fields.One2many(
        "pico.workflow.version", "workflow_id", string="Child Versions"
    )
    ...
    def pico_subscribe(self):
        ...
        try:
            api.subscribe_jsonrpc(
                base_url + "/picoapi/webhook",
                "newWorkflowVersionMethod",
                "workOrderCompleteMethod",
            )
            ...
            raise UserError(
                "Webhook Subscribe resulted in an error from Pico Endpoint URL (%s)"
                % (api.url,)
            )
    ...
    def validate_bom_setup(self, boms=None, should_raise=False):
        ...
        for workflow in self:
            if not boms:
                ...
                processes = (
                    self.env["pico.workflow.process"]
    ...
    def _validate_bom_setup(self, bom):
        ...
        lines = bom.bom_line_ids.filtered(
            lambda l: l.pico_process_id and not l.pico_process_id.active
        )
        if lines:
        ...
        process_attrs_to_consume = bom.pico_process_id.process_ids.mapped(
            "attr_ids"
        ).filtered(lambda a: a.type == "consume")
        ...
        lines = bom.bom_line_ids.filtered(
            lambda l: l.product_id.tracking in ("lot", "serial") and not l.pico_attr_id
        )
        if lines:
            raise PicoBoMNeedsMap(
                "Not all lot/serialized items are mapped to a pico attribute."
            )
    ...
    def _sync_data(self, values):
        ...
        if not workflow_pico_id:
            raise ValidationError(
                'Cannot create Pico Workflow without an "id" in response.'
            )
    ...
    def _get_values_from_pico_data(self, values):
        return {"name": values.get("name", ""), "pico_id": values["id"]}
    ...
    def _reconcile_processes(self, process_list):
        ...
        existing_processes = original_processes.filtered(
            lambda p: p.pico_id in process_dict
        )
        ...
        if line_commands:
            ...
            line_commands = []
            pico_producing = (
                self.process_ids.browse()
            )  # Empty record to use .id == False
            for process in self.process_ids.sorted(
                lambda p: -p.sequence
                ...
                if process.attr_ids.filtered(lambda a: a.type == "produce"):
                    ...
                    if process.producing_process_id:
                        line_commands.append(
                            (1, process.id, {"producing_process_id": False})
                        )
                    pico_producing = process
                else:
                    if process.producing_process_id != pico_producing:
                        line_commands.append(
                            (1, process.id, {"producing_process_id": pico_producing.id})
                        )
            if line_commands:
                self.write({"process_ids": line_commands})
    ...
    def _reconcile_process_attrs(self, odoo_process, process):
        ...
        original_attrs = (
            odoo_process
            and odoo_process.attr_ids
            or self.env["pico.workflow.process.attr"]
        )
...
class PicoMESProcess(models.Model):
    ...
    attr_ids = fields.One2many(
        "pico.workflow.process.attr", "process_id", string="Attrs"
    )
    sequence = fields.Integer("Sequence", default=1)
    producing_process_id = fields.Many2one(
        "pico.workflow.process", string="Producing Process"
    )

    workflow_id = fields.Many2one("pico.workflow", string="Parent Workflow")

    process_ids = fields.One2many(
        "pico.workflow.process",
    ...
    def _compute_process_ids(self):
        for process in self:
            process.process_ids = process.workflow_id.process_ids.filtered(
                lambda p: p == process or p.producing_process_id == process
            )
...
class PicoMESProcessAttr(models.Model):
    ...
    type = fields.Selection(
        [("produce", "Produce"), ("consume", "Consume"), ("other", "Other")],
        string="Type",
        default="other",
    )
    name = fields.Char("Name")
